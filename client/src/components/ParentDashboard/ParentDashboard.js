...
//import ParentGrades from '../ParentGrades/ParentGrades';
...
export class ParentDashboard extends Component {
    ...
    async componentDidMount() {
        ...
        if(this.props) {
            ...
            const response = await fetch(`http://localhost:8080/project/parent/logged`, {
                mode: 'cors',
                method: 'GET',
                headers: {
    ...
    render() {
        ...
        /*const { isLoading, isError, loggedUser } = this.state
        ...
        return (
                        ...
                        <div className="profile_info" style={{float: "left", textAlign: "left"}}>
                            <img src={this.state.loggedUser.gender === "GENDER_MALE" ? male : female} alt="User-face" />
                                <div>
                                    ...
                                    <br/>
                                    <small>
                                        <Link to={{ pathname: "/parents/parentprofile" }}>edit</Link>
                                    </small>
                                </div>
                        </div>
                    </div>
            ...
            <>
                <div className="navigation">
                    <Link to={{ pathname: "/parents/childrengrades" }}>--Children Grades-</Link>
                    <Link to={{ pathname: "/parents/parentprofile" }}>-Parent profile-</Link>
                </div>
                <Switch>                    
                    <PrivateRoute
                        isLoggedIn={this.state.isLoggedIn}
                        isAuthorized={userRole === "ROLE_PARENT"}
                        path="/parents/childrengrades" 
