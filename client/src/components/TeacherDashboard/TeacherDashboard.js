...
export class ParentDashboard extends Component {
    ...
    async componentDidMount() {
        ...
        if(this.props) {
            ...
            const response = await fetch(`http://localhost:8080/project/teacher/logged`, {
                mode: 'cors',
                method: 'GET',
                headers: {
            ...
            if( !this.state.isCatchError && response.ok ) {
                ...
                user.employmentDate = user.employmentDate/*.toLocaleString("sr", {timeZone: "Europe/Belgrade"})*/.split("-").reverse().join("-");
