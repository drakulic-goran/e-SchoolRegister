...
const StudentForm = lazy(() => import('../StudentForm/StudentForm'));
...
class StudentProfil extends Component {
    ...
    handleDelete = async () => {
        ...
        const response = await fetch('http://localhost:8080/project/student/'+this.props.loggedUser.id, {
            mode: 'cors',
            method: 'DELETE',
            headers: {
    ...
    handleCancel = () => { this.props.history.goBack() /*.push(this.props.linked)*/ }
                ...
                <React.Suspense fallback={<h1><FontAwesomeIcon icon={faSpinner} spin /></h1>} id='susp'>
                    ...
                    { loggedUser && 
                            <StudentForm 
                                ...
                                onStudentSubmit = {this.handleEdit}
                                id='sf'
                            /> 
                    }
